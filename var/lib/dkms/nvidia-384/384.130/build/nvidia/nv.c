...
static void nv_init_ibmnpu_info(nv_state_t *nv)
{
#if defined(NV_PNV_PCI_GET_NPU_DEV_PRESENT)
    nv_linux_state_t *nvl = NV_GET_NVL_FROM_NV_STATE(nv);

    // Look for attached NPU devices until there aren't any left
    for (nvl->attached_npu_count = 0;
         nvl->attached_npu_count < NV_MAX_ATTACHED_IBMNPUS;
         nvl->attached_npu_count++)
    {
        nvl->npu_devs[nvl->attached_npu_count] =
            pnv_pci_get_npu_dev(nvl->dev, nvl->attached_npu_count);

        if (nvl->npu_devs[nvl->attached_npu_count] == NULL)
        {
            break;
        }
    }
...
static int nv_init_ibmnpu_devices(nv_state_t *nv)
{
#if defined(NVCPU_PPC64LE)
    NvU8 i;
    nv_linux_state_t *nvl = NV_GET_NVL_FROM_NV_STATE(nv);

    for (i = 0; i < nvl->attached_npu_count; i++)
    {
        nv_printf(NV_DBG_SETUP,
            "NVRM: initializing IBM-NPU device %04x:%02x:%02x.%u"
            " associated with GPU device " NV_PCI_DEV_FMT "\n",
            NV_PCI_DOMAIN_NUMBER(nvl->npu_devs[i]),
            NV_PCI_BUS_NUMBER(nvl->npu_devs[i]),
            NV_PCI_SLOT_NUMBER(nvl->npu_devs[i]),
            PCI_FUNC(nvl->npu_devs[i]->devfn),
            NV_PCI_DEV_FMT_ARGS(nv));
        if (ibmnpu_init_device(nvl->npu_devs[i]) != NVL_SUCCESS)
        {
            return -EIO;
        }
    }
...
)
{
     ...
     */
    if (nvl->attached_npu_count && !(nv->nvlink_sysmem_links_enabled))
